...
from unetrpp.layers import LayerNorm
from unetrpp.transformerblock import TransformerBlock
from unetrpp.dynunet_block import get_conv_layer, UnetResBlock


einops, _ = optional_import("einops")

class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[96 * 80, 48 * 40, 24 * 20, 12 * 10],dims=[32, 64, 128, 256],
    ...
    def apply_stage(self, x, i, mask=None, patch_size=16):
    ...
    def forward_features(self, x, mask=None):
        ...
        if mask is None:
            ...
            patch_size = 8
            x = self.apply_stage(x, 0, mask=mask, patch_size=patch_size)
            patch_size //= 2
        ...
        for i in range(1, 4):
            ...
            if mask is None:
                ...
                x = self.apply_stage(x, i, mask=mask, patch_size=patch_size)
                patch_size //= 2
    ...
    def forward(self, x, mask=None):
        x, hidden_states = self.forward_features(x, mask=mask)
        return x, hidden_states
